...
def PDFs(y, data, label, col, style='', S = 1.47396990069):
	...
	print(max_)
	if style == '':
		p= plt.plot(y - S, K * math.sqrt(12), 'r', label = label, color = col) 
	else:
		p= plt.plot(y - S, K * math.sqrt(12), 'r', label = label, color = col,linestyle = style)
	return p
